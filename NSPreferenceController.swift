...
class NSPreferenceController: NSWindowController, NSWindowDelegate, NSToolbarDelegate, PreferenceWindowDelegate {
    ...
    init(panes: [PreferencePane]) {
        ...
        let window = NSWindow(contentRect: NSRect(origin: origin, size: size), styleMask: [.closable, .miniaturizable, .titled], backing: .buffered, defer: false)
        ...
        window.toolbar!.insertItem(withItemIdentifier: window.toolbar!.centeredItemIdentifier!, at: 0)
    ...
    internal func toolbar(_ toolbar: NSToolbar, itemForItemIdentifier itemIdentifier: NSToolbarItem.Identifier, willBeInsertedIntoToolbar flag: Bool) -> NSToolbarItem? {
        if itemIdentifier == toolbar.centeredItemIdentifier {
            ...
            let control = NSSegmentedControl(labels: labels, trackingMode: .selectOne, target: self, action: #selector(self.changeTab))
            control.selectSegment(withTag: index)
            item.view = control
    ...
    internal func getWindowRect(forNewIndex newIndex: Int) -> NSRect {
        ...
        let newFrameSize = NSMakeSize(titleSize.width + newContentSize.width,
